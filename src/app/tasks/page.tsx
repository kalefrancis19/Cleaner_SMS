...
export default function TasksPage() {
  ...
  const formatScheduledTime = (task: any) => {
    ...
    return task.estimatedTime || 'No scheduled time';
  };
              ...
              {Object.entries(groupedTasks).map(([propertyKey, propertyData]) => {
                ...
                return (
                  <div key={propertyKey} className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 overflow-hidden">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <p className="font-bold text-gray-900 dark:text-white text-lg">
                            {property.estimatedTime || propertyTasks.reduce((total, task) => {
                              const timeStr = task.estimatedTime || '0 hours';
                              const hours = parseFloat(timeStr.match(/(\d+(?:\.\d+)?)/)?.[1] || '0');
                              return total + hours;
                            }, 0).toFixed(1) + ' hours'}
                          </p>
                        </div>
                    ...
                    {/* Property Tasks */}
                    <div className="p-6 bg-gray-50/30 dark:bg-gray-700/30">
                      <h4 
                        className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center cursor-pointer hover:text-blue-600 transition-colors"
                      ...
                      {expandedProperties.has(propertyKey) && (
                        <div className="space-y-3">
                          {propertyTasks.map((task) => (
                            <div key={task._id || task.id} className="bg-white/60 dark:bg-gray-600/60 rounded-xl p-4 border border-gray-100 dark:border-gray-500">
