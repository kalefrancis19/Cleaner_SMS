...
${scoring.missedRequirements.length > 0 ? `⚠️ Areas for Improvement:\n${scoring.missedRequirements.map(req => `• ${req}`).join('\n')}` : ''}`;
  ...
  getEnhancedFallbackResponse(userMessage) {
    ...
    if (lowerMessage.includes('property') || lowerMessage.includes('where') && lowerMessage.includes('start')) {
      if (currentProperty) {
        return `Great question! 🏠 For ${currentProperty.name}, I recommend starting with the ${currentProperty.roomTasks?.[0]?.roomType || 'first room'} to take before photos. This helps us track your progress and ensure quality. Would you like to begin there?`;
      } else {
        return "I'd be happy to help you get started! 🏠 First, let me know which property you're working on, and I'll guide you through the cleaning process step by step.";
      }
    }
    
    // Handle progress/status inquiries
    if (lowerMessage.includes('where') && (lowerMessage.includes('am') || lowerMessage.includes('progress'))) {
      return this.getCurrentProgressStatus();
    }
    
    if (lowerMessage.includes('status') || lowerMessage.includes('progress') || lowerMessage.includes('how far')) {
      return this.getCurrentProgressStatus();
    }
    
    if (lowerMessage.includes('what') && (lowerMessage.includes('next') || lowerMessage.includes('should') || lowerMessage.includes('do'))) {
      return this.getNextSteps();
    }
    
    // Handle room-specific questions
    if (lowerMessage.includes('room') || lowerMessage.includes('bedroom') || lowerMessage.includes('bathroom') || lowerMessage.includes('kitchen')) {
      return this.getRoomSpecificInfo(lowerMessage);
    }
    
    // Handle photo-related questions
    if (lowerMessage.includes('photo') || lowerMessage.includes('picture') || lowerMessage.includes('image')) {
      return this.getPhotoGuidance();
    }
    
    // Handle manual/requirements questions
    if (lowerMessage.includes('manual') || lowerMessage.includes('requirement') || lowerMessage.includes('what to do')) {
      return this.getManualRequirements();
    }
    
    // Handle scoring/quality questions
    if (lowerMessage.includes('score') || lowerMessage.includes('quality') || lowerMessage.includes('grade')) {
      return this.getScoringInfo();
    }
    
    // Handle casual conversation and goodbyes
    if (lowerMessage.includes('bye') || lowerMessage.includes('goodbye') || lowerMessage.includes('see you')) {
      return "Goodbye! 👋 Feel free to come back anytime if you need help with cleaning tasks. Have a great day!";
    }
    
    if (lowerMessage.includes('thanks') || lowerMessage.includes('thank you') || lowerMessage.includes('thx')) {
      return "You're welcome! 😊 I'm here to help make your cleaning tasks easier. Let me know if you need anything else!";
    }
    
    if (lowerMessage.includes('ok') || lowerMessage.includes('okay') || lowerMessage === 'k') {
      return "Got it! 👍 I'm ready when you are. Just let me know what you'd like to work on next.";
    }
    
    if (lowerMessage.includes('finish') && lowerMessage.includes('clean')) {
      return "Great! 🎉 When you're done cleaning, just ping me and I'll help you with the final documentation and scoring. Take your time!";
    }
    
    if (lowerMessage.includes('ping') || lowerMessage.includes('call') || lowerMessage.includes('message')) {
      return "Absolutely! 📱 I'll be here when you're ready. Just send me a message and I'll help you continue with the cleaning process.";
    }
